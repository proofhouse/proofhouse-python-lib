...
# Shared docker-run prefix. DOCKER_CONFIG points at a fresh empty
...
# The two builds land in separate temp dirs. Empirical backstop for the
# reproducibility contract above; runs without touching dist/.

# Build twice and fail if the wheel or sdist digests differ.
[script]
build-repro-check:
    out_a=$(mktemp -d)
    out_b=$(mktemp -d)
...
# The read-only counterpart `lint-ruff-format` runs in CI and the
# aggregator.

# Format Python source in place via ruff's formatter.
...
# Format TOML files in place with tombi 1.2.0.
format-toml:
    tombi format

# Read-only twin is `lint-just`. --unstable is required because `--fmt`
# is still gated behind just's unstable flag; the `set unstable` at the
# top of the file governs recipe attributes, not command-line
# subcommands, so the flag has to be passed here as well. The one
# rewrite adopting this gate cost was collapsing that pair of `:= true`
# settings to their bare form — 1.51.0 (the version CI installs) and the
# current Homebrew release agree on the result, so the gate reads the
# same on both sides.

# Rewrite this Justfile in just's canonical formatting.
format-just:
    just --fmt --unstable

# --- Fix ---

# The order matters: `check --fix` can leave code the formatter wants to
# reflow (dropped arguments, collapsed branches), so the format pass
# runs last.

# Apply ruff's auto-fixes, then format.
fix *args:
...
# CI's lint job invokes only this recipe, so wiring up a new gate means
# appending one dependency here instead of editing workflow YAML.

# Run every Python source gate plus actionlint.
...
# Read-only twin of `just format`. The path-less invocation
# deliberately walks the whole tree, tests included — `[tool.ruff] src`
# names import-resolution roots, not scan scope.

# Check that ruff's formatter would change nothing.
lint-ruff-format *args:
    uv run ruff format --check {{ args }}

# [tool.ruff.lint] selects ALL with documented exemptions. The
# path-less invocation deliberately walks the whole tree, tests
# included — `[tool.ruff] src` names import-resolution roots, not scan
# scope.

# Lint Python source against the ruff ruleset in pyproject.toml.
lint-ruff *args:
    uv run ruff check {{ args }}

# [tool.pyrefly] in pyproject.toml starts from the strict preset and
# pins every diagnostic kind to error severity, so this gate fails on
# any finding pyrefly can produce, untyped defs included.

# Type-check src and tests with pyrefly.
lint-types *args:
    uv run pyrefly check {{ args }}

# Any score above the ceiling fails the run. Scope and ceiling come
# from [tool.complexipy] in pyproject.toml.

# Score every function's cognitive complexity with complexipy.
lint-complexity:
    uv run complexipy

# vulture pairs every name defined under the [tool.vulture] paths in
# pyproject.toml against the names used there. With src and tests in
# one scan, the tests stand in for downstream callers of the published
# API, so this gate asks "does anything exercise this?" of every public
# name.

# Report unused definitions with vulture.
lint-deadcode:
    uv run vulture

# The similarities checker is the only message the [tool.pylint] tables
# in pyproject.toml leave enabled. pylint accepts the directories to
# scan solely as command-line arguments, which makes this recipe the
# home of the src and tests scope.

# Find copy-pasted logic with pylint's similarities checker.
lint-dup-code:
    uv run pylint src tests

# Every tracked file must name Apache-2.0 and its copyright holder,
# either inline (.py sources keep their two-line SPDX headers) or
# through the bulk annotations in REUSE.toml (configs, the lockfile,
# formats with no comment syntax). Serial on purpose: at a few dozen
# files the default worker pool only adds startup cost, and the serial
# path also works in locked-down dev sandboxes that refuse the pool's
# named semaphores.

# Check the tree against the REUSE specification.
lint-reuse:
    uv run reuse --no-multiprocessing lint

# The [tool.importlinter] contracts in pyproject.toml keep the
# expression pipeline stages layered, and none of them imports the
# shipped testing helpers. The bare command is import-linter's own CLI,
# not this recipe recursing.

# Verify the package's import graph with import-linter.
lint-imports:
    uv run lint-imports

# ruff already carries the S (flake8-bandit) rules under its
# `select = ["ALL"]` set, but the two are not the same scanner: ruff
# reimplements a subset of bandit's checks against its own syntax tree,
# while bandit keeps the full upstream catalog — B-prefixed test IDs
# ruff has never ported — and tracks new advisory patterns on its own
# release cadence. Running both pairs the fast in-editor S diagnostics
# with the deeper second read that notices what the port skipped.
# `-c pyproject.toml` loads the [tool.bandit] table; `-r src` walks the
# package recursively. This is the analog of the Go twin enabling gosec
# inside golangci-lint, so it rides the same lint gate rather than a
# standalone job — fast enough to block a merge from the lint set, where
# its non-zero exit on any finding stops the branch.

# Run bandit's static security analysis over the shipped source.
lint-bandit:
    uv run bandit -c pyproject.toml -r src

# Glob excludes the LICENSE (canonical Apache 2.0 text), the
# auto-generated changelog, vale's own style packages, scratch dirs, the
# gitignored agent worktrees under .claude/worktrees/ (whose nested
# virtualenvs vale would otherwise crawl), the COMMIT_AGENTMSG draft
# (the `lint-commit-msg` recipe owns that one under the stricter commit
...
# cspell ignores binaries, generated files, the virtualenv, and build
# output via the ignorePaths block in .cspell.jsonc. The
...
# tombi is the org TOML gate (tombi 1.2.0): it lint-checks every tracked
# *.toml. Cargo.toml/pyproject.toml validate offline against embedded
# SchemaStore schemas; cog.toml, .rumdl.toml, REUSE.toml, deny.toml et
# al. get syntax + style checks. We run the format gate in
...
# Lint and format-check every tracked TOML file with tombi.
lint-toml:
...
# The rules cover charset, line endings, final newline, trailing
# whitespace, indent style and size. The other gates each own one
# language; this one holds the baseline that spans all of them,
# including the file types no linter in the set reads (.gitattributes,
# .python-version, the LICENSES texts).
...
# workflow-shape rules yamllint can't see. Runs from the digest-pinned
# Docker image declared at the top of this file; Renovate bumps the
# version + digest via the shared Justfile customManager.

# Lint GitHub Actions workflow files via actionlint.
lint-workflows:
    {{ actionlint }}

# The same gates the commit-msg hook runs fire here, so message
# problems surface while iterating rather than at commit time. Reads
# the draft from the repo-root COMMIT_AGENTMSG file (gitignored; see
...
# The property suite under tests/property loads hypothesis's `dev`
# profile by default (50 examples, a fast inner loop); set
...
# A bare `--cov` reads `source` from [tool.coverage.run], so the package
# gets measured rather than the tests; `--cov-branch` records which arm
# of each fork ran. The report and the fail_under threshold come from
# [tool.coverage.report]. This is the inner loop: run it, read the
# Missing column, write the test that reaches the unreached arm.

# Run the suite under branch coverage and hold it to the floor.
cover:
    uv run pytest --cov --cov-branch

# The recipe prints where the report lands. Each statement and branch
# arm is shaded by whether a test reached it, which pins down the exact
# line a new test still owes.

# Build the line-by-line HTML report under htmlcov/.
cover-html:
    uv run pytest --cov --cov-branch
    uv run coverage html
    @echo "open htmlcov/index.html"

# That format is what diff-cover reads and what the CI upload action
# publishes, so this assumes a `cover` or `cover-slot` run produced the
# data first.

# Write Cobertura XML from whatever .coverage already holds.
cover-xml:
    uv run coverage xml -o coverage.xml

# The whole-tree floor already sits at 100%, so on a clean branch this
# adds nothing; it earns its keep by catching a diff that strips
# coverage from edited lines before the slower combined total
# recomputes in CI. Reads coverage.xml, so run `cover-xml` first (CI
# does).

# Fail when a line touched since [base] is not covered.
cover-diff base="origin/main":
    uv run diff-cover coverage.xml --compare-branch={{ base }} --fail-under=100

# The suite does not rerun; handy after editing exclude_also to confirm
# the total still holds without paying for another run.

# Re-read the existing .coverage data and re-check the threshold.
cover-check:
    uv run coverage report

# This is the binding gate: a branch no single platform happens to run
# still has to be reached somewhere, and the merged report is the proof.
# The CI coverage job runs this after pulling down the per-slot
# artifacts.

# Fold every slot's data into one report, hold the floor, render XML.
cover-combine:
    uv run coverage combine
    uv run coverage report --fail-under=100
    uv run coverage xml -o coverage.xml

# COVERAGE_FILE tags the data file with the slot so the downstream job
# can combine every slot losslessly; --cov-fail-under=0 hands the
# threshold off to that combined check, since one slot need not carry
# the whole library on its own. CI passes the os/python pair as the slot
# name. `-n auto` spreads the run across the slot's cores: pytest-cov
# sums each worker's tally back into the one COVERAGE_FILE, so the
# slot's number stays whole under the split.

# Record one matrix slot's coverage and render its Cobertura.
[script]
...
# Point it at the file you just touched — the precedence table in
# parser.py, the Fraction arithmetic in evaluator.py, the bracketing
# rules in formatter.py — and cosmic-ray rewrites one construct at a
# time, replays the suite, and files each mutant KILLED, SURVIVED, or
# incompetent. The SURVIVED lines are where the code branches but no
# assertion reads the difference. It clones cosmic-ray.toml into a
# scratch config with the path and budget overridden, runs the pragma
# filter so the equivalent mutants flagged in the source drop out before
# they cost a run, then prints the surviving set and the rate. The
# whole-package form below feeds the nightly.

# Mutate a single module for a tight edit-rerun loop.
[script]
...
# The scheduled workflow calls this and so can anyone vetting a
# release-bound change, which is why it lives in one recipe rather than
# being inlined into the YAML — the fuzz pair keeps the same split.
# module-path and the equivalent-mutant exclusions ride along from
# cosmic-ray.toml; only the budget gets overridden. The pragma filter
# retires the type-alias mutants the source marks before exec spends
# time on them. This recipe sets no passing bar: it lists what lived and
# exits zero whatever the rate, leaving the score that can block a merge
# to the diff-scoped check. Read the surviving block to find the next
# assertion worth writing.

# Sweep every shipped module.
[script]
mutate-all:
    mkdir -p .cosmic-ray
...
# mutscope.py reads the diff from [base] to HEAD, takes the changed
# library modules plus the ones that import them through the graph,
...
# Confine the sweep to a branch's changes and refuse any survivor.
[script]
...
# The run fails if the search turned up a counterexample. Where the
# example-based `just test` run draws a fixed number of cases per
# property, HypoFuzz steers its inputs by the branches they reach, so it
# keeps probing the precedence climber, the Fraction evaluator, and the
# formatter's bracketing for the input no fixed draw stumbles onto. The
# budget comes from FUZZ_TIME — a count of seconds or a 30s/5m/1h
# duration, the same dial the Go twin's fuzz recipe reads — and defaults
# short for an edit-rerun loop; the nightly hands in a longer one.
# tools/fuzz.py owns the budget and the verdict: HypoFuzz has no time
# flag and stops on its own only once every target has failed, so the
# script stops it on the clock and then replays the suite under pytest,
# which reruns any saved falsifying example from the .hypothesis
# database and exits non-zero on a reproduction. Both the inner loop and
# the scheduled sweep call this one recipe, the split the mutation pair
# above keeps too.

# Search the property suite with HypoFuzz for a bounded stretch.
fuzz:
    uv run python tools/fuzz.py

# --- Security ---

# `gitleaks git` diffs each commit against the binary's bundled regex
# and entropy rules; a hit reports the file, line, commit, and rule,
# enough to trace the leak without a second pass. A credential baked
# into a published wheel ships to every installer and lingers in the
# sdist's packed history, so the full scan guards what a release would
# otherwise hand out. The scan runs from a digest-pinned image, so the
# rule set moves only when Renovate bumps the version and digest pair.
# This one stays a local recipe on purpose: GitHub's own secret scanning
# with push protection holds the same line on the hosted side, so no CI
# job duplicates it.

# Scan the working tree and every commit in history for secrets.
gitleaks:
    {{ gitleaks_scan }} git --verbose .

# The subject is the lock, not the live `.venv`: pip-audit
...
# Audit the dependency closure against OSV and PyPI advisories.
[script]
audit:
    work=$(mktemp -d)
...
# Run the scanners that vet what the package ships.
...
# The set covers commit-msg, pre-commit, and pre-push. `just setup` runs
# this automatically; it stays a separate recipe so contributors can
